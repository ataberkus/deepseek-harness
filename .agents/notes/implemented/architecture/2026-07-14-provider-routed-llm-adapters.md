...
The adapter calls pi-ai's `streamSimple()` so each catalog model chooses its registered API implementation, including OpenAI Responses instead of Chat Completions where the descriptor says `openai-responses`. Harness temperature, maximum tokens, signal, session id, and the profile's common stream options flow through directly. Profile headers merge with Harness-owned headers, which win case-insensitive collisions: attribution identifies the client on every route, while OpenCode Go hashes the request's Session id with a domain-separated SHA-256 digest and sends the result as its required `x-opencode-session` routing header across protocol implementations. The raw Session id stays local. The adapter maintains no DeepSeek-specific payload rewrites or provider-protocol matrix.
