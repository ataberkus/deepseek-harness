...
`packages/llm/llm-pi-ai/tests/listing.spec.ts` and `live-catalog.spec.ts` pin live-only reasoning overlay and `resolveModelInfo` efforts. `tests/cursor.spec.ts` pins fallback ids, live-first merge, documented Fast only, and inferred reasoning. `packages/llm/llm/tests/service.spec.ts` pins default `UNSUPPORTED_OPTION` and adapter-owned logout. `packages/host/apiproxy/tests` pin `llm.logout` round-trip and `oauth-logout-failed`. `packages/client/ui-settings-models/tests` pin Sign out and fetch search / select-all.
