...
`apply` constructs `ConversationController` on its owning plugin fiber before registering slot entries. Render-time injectors therefore see the service as soon as the plugin becomes active, and entry effects are removed before the provider is disposed.
