...
A stored `openai-codex` oauth credential injects a settings-free live route into the adapter registry so the model picker can list the account's models: the live registry merged over pi-ai's catalog ([live listing](2026-09-12-codex-oauth-model-listing.md)). The configurable-provider directory still withholds the OAuth-only catalog **key card**, which is the withholding note's decision; a settings-stored profile still appears there so it can be edited or deleted. The live route names no `apiKeyEnv`, so first-run onboarding still requires a usable API-key provider until Codex login. The Models page shows that injected route as a signed-in row (name plus connected dot, Sign out via `llm.logout`, no editor, no Sign-in button). `/logout openai-codex` remains the command equivalent.

`Provider is not configured` maps to `LlmError('MISSING_CREDENTIAL')` and names `/login openai-codex`. Hosted `cursor` and `google-gemini-cli` logins are sibling routes on the same store and command table; those unofficial transports are [the Cursor OAuth host](2026-08-18-cursor-oauth-host.md) and [the Gemini CLI OAuth host](2026-08-19-google-gemini-cli-oauth-host.md).
...
**Un-withholding `openai-codex` as a key card on the Models page.** Rejected: a key field still cannot complete ChatGPT login. A signed-in connected row is not that card: it reports OAuth state the host already knows, without a Sign-in control or an editable key. Sign out deletes the stored login; it does not add a key field.

## Consequences

A CLI, ACP, or Web session can run `/login openai-codex`, complete ChatGPT browser login, and select an `openai-codex` model. Settings → Models then shows the route as signed in with ChatGPT. The agent loop, tools, approvals, and session log stay harness-owned; pi-ai owns PKCE, refresh, and Codex Responses. Codex Responses transport stays pi-ai, so a wire change there updates the pi-ai adapter; the account registry reader is harness-owned ([live listing](2026-09-12-codex-oauth-model-listing.md)) and tracks registry shape drift.
