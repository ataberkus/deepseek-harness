...
`dsh-llm-pi-ai` 把 Gemini CLI OAuth 托管在 [`FileOAuthStore`](../../../../packages/llm/llm-pi-ai/src/oauth-store.ts) 上，与 [OpenAI Codex](2026-08-18-openai-codex-oauth-host.zh.md) 和 [Cursor](2026-08-18-cursor-oauth-host.zh.md) 共用同一份存储。托管表是 `openai-codex`、`cursor`、`google-gemini-cli`。空的 `/login` 仍表示 `openai-codex`。其他名字失败。一次进行中的登录覆盖整个宿主，因此 Gemini 回环、Codex PKCE 回调与 Cursor 轮询不能竞态。Web 客户端也会为 `/login google-gemini-cli` 打开空白标签。宿主通过 `commands/open-url` 转发授权 URL，只有该事件没有订阅者时才使用操作系统浏览器，因此一次 Web 登录不会再打开第二个浏览器。回环页面只确认收到 Google 的授权响应；`/login` 要等换票、项目发现和 `models.login` 写入凭据后才报告成功。
...
`tests/google-gemini-cli.spec.ts` 钉住公开客户端授权 URL、换票、项目发现（已有 companion、onboard 加 LRO、Workspace 的 `GOOGLE_CLOUD_PROJECT`、VPC-SC）、保留 `projectId` 的刷新、回环回调（成功、拒绝、缺 code、中止、二次绑定、结算后再来一次回调）、中性的回环页面文案、请求信封与 thinking 档位映射，以及把文本、thinking 和 `functionCall` SSE 映射到 streamSimple——从不访问真实 Google API。`tests/oauth-login.spec.ts` 钉住 `/login google-gemini-cli`／`/logout google-gemini-cli`、空输入仍是 Codex、无目录卡片的启动注入、settings 声明的 `google-gemini-cli` 不是 `auth: oauth`，以及 `MISSING_CREDENTIAL` 点名 `/login google-gemini-cli`。`tests/catalog.spec.ts` 保留目录不予提供 `google-gemini-cli`，除非 settings profile 点名它。模型 UI 测试钉住 Gemini CLI 已登录文案。包测试覆盖登录文本；无密钥的装配快照无法重放 Google 登录。
