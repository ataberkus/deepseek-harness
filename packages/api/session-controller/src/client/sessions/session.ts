...
export class Session implements SessionFace {
  ...
  async edit(
    ...
    return this.remote.session.edit({
      sessionId: this.sessionId,
      messageSeq,
      checkpointId,
      text,
  ...
  async activate(): Promise<RemoteResult<{
    ...
    return this.remote.session.activate({ sessionId: this.sessionId })
  }
   ...
   * @param frame - queue or checkpoint replacement addressed to this Session.
   */
  handleControlFrame(
    frame: Extract<SessionControlFrame, { type: 'queue' }>
      | Extract<SessionControlFrame, { type: 'session/checkpoints' }>,
  ): void {
