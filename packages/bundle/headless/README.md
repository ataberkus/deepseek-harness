...
The final answer remains stdout-only for script consumers. If the session has priced usage, stderr adds `dsh: Estimated session cost: $…`; Cursor's input estimate is labeled `Approx.` and missing pricing reports `dsh: Estimated session cost unavailable`.
