...
`ConversationSnapshot.checkpoints` 按完整快照镜像 `session/checkpoints` 帧，包括检查点行、已应用检查点、分支标签、工作区可恢复性、当前 edit/activate 操作以及 recovery-required 诊断。`SessionManager` 会在 Session 对象打开前保留这份普通快照，并把分支标签与可恢复性投影到 Session 列表摘要；`session/subscribed` 清除上一代状态，随后到达的完整 baseline 替换它，而检查点 blob 永远不会进入浏览器。
