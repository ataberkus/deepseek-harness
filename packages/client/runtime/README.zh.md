...
`ConversationSnapshot.checkpoints` 按完整快照镜像 `session/checkpoints` 帧，包括实时 `enabled` 标志、检查点行、已应用检查点、编辑子会话的源谱系检查点、分支标签、工作区可恢复性、当前 edit/activate 操作以及 recovery-required 诊断。`enabled: false` 的快照会清除客户端入口和派生列表标签，同时保留元数据投影以便重连；`SessionManager` 只在启用时把分支标签和可恢复性投影到 Session 列表摘要。`session/subscribed` 清除上一代状态，随后到达的完整 baseline 替换它，而检查点 blob 永远不会进入浏览器。
