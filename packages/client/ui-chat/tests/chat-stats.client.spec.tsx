...
import {
  StatsLine, deriveStats, formatCost, formatDuration, type StatsLineProps,
...
describe('StatsLine', () => {
  ...
  it('adds connected subagent spend to the owning session cost', () => {
    ...
    const useSessions: NonNullable<StatsLineProps['useSessions']> = selector => selector({
      byId: {
        [root]: { id: root },
        [child]: {
          id: child,
          parentId: root,
          origin: 'subagent',
          projectionValues: { tokenUsage: { costUsd: 0.25 } },
        },
