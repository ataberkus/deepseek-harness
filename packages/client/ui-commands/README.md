...
On `/login`, `/login openai-codex`, or `/login cursor`, this browser opens a blank tab during the keystroke and navigates it when the Host forwards `commands/open-url`, because a popup blocker swallows a `window.open` that runs after the authorize URL arrives. Non-https URLs are ignored. The named tab is reused so a second `/login` does not replace an in-progress authorize page with a blank one.
