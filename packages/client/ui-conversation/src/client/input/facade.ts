...
import type {
  ArbitrateKey, ArbitrateOutcome, CommandClaim, ConsumeTokenRequest, ConversationEditDraft,
...
export class SessionInputShell implements SessionInput {
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    if (this.snapshot.draft.trim() === '' && this.imageIds.length > 0) {
      if (this.snapshot.phase === 'plain') {
        ...
        void this.invokeDefaultSink('', imageIds, mode, controller.signal).then((outcome) => {
  ...
  private sinkSerialized(
    ...
    if (occurrences.length === 0) {
      this.settleSink(attempt, this.invokeDefaultSink(draft.trim(), imageIds, mode, attempt.signal))
      return
    }
    const inputTriggers = this.deps.inputTriggers?.()
    void Promise.all(occurrences.map(async (o) => {
      ...
      (parts) => {
        ...
        this.settleSink(attempt, this.invokeDefaultSink(out.trim(), imageIds, mode, attempt.signal))
      },
      (error: unknown) => {
