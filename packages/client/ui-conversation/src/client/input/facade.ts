...
import type {
  ArbitrateKey, ArbitrateOutcome, CommandClaim, ComposerKeyboard, ConsumeTokenRequest, ConversationEditDraft, DraftAttachmentId,
...
export class SessionInputShell implements SessionInput {
  ...
  /** Message/checkpoint edit retained while the edited send is admitted. */
  ...
  /** Cancel edit mode and restore the draft that was present on entry. */
  cancelEdit(): void {
    const target = this.editTarget
    this.editTarget = undefined
    if (target !== undefined) {
      this.setDraft(target.previousDraft ?? '')
      return
    }
    this.publish()
  }
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    if (this.snapshot.draft.trim() === '' && this.attachmentIds.length > 0) {
      if (this.snapshot.phase === 'plain') {
        ...
        void this.invokeDefaultSink('', attachmentIds, mode, controller.signal).then((outcome) => {
  ...
  /** Route the default sink through the active edit target when one is staged. */
  private invokeDefaultSink(
    ...
    return edit === undefined
      ? this.deps.defaultSink(text, attachmentIds, mode, signal)
      : this.deps.defaultSink(text, attachmentIds, mode, signal, edit)
  }
  ...
  private sinkSerialized(
    ...
    if (occurrences.length === 0) {
      this.settleSink(attempt, this.invokeDefaultSink(draft.trim(), attachmentIds, mode, attempt.signal))
      return
    }
    const inputTriggers = this.deps.inputTriggers?.()
    void Promise.all(occurrences.map(async (o) => {
      ...
      (parts) => {
        ...
        this.settleSink(attempt, this.invokeDefaultSink(out.trim(), attachmentIds, mode, attempt.signal))
      },
      (error: unknown) => {
