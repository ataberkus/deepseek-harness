...
import type {
  ComposerKeyboard, ConversationEditDraft, DraftAttachmentId, DraftAttachmentSerializationResult, InputTriggerController,
...
export class InputHub implements SessionInputResolver {
  ...
  shellFor(binding: SessionBinding): SessionInputShell {
    ...
    const shell = new SessionInputShell({
      ...
      defaultSink: (text, attachmentIds, mode, signal, edit) => this.sink(session, text, attachmentIds, mode, signal, edit),
  ...
  private sink(
    ...
    if (edit !== undefined) {
      if (attachmentIds.length > 0) {
        return Promise.resolve({ kind: 'error', text: this.t('message.editAttachmentsUnsupported') })
      }
