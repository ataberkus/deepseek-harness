...
在 composer 菜单中为模型加星即可置顶：收藏的模型在模型位中以收藏分组排在最前，在 `/model` 弹窗中同样排在最前。收藏按浏览器经 `localStorage` 持久化，仅影响呈现——选择与路由仍使用提供方／模型／推理强度 id。星标只能在 composer 模型位中切换；弹窗在下次打开时反映最新顺序。
