...
提供方自有登录在未连接时显示为“连接”卡片，连接后显示为带断开入口的 live 行。Codex、Cursor 与 Antigravity 运行命名空间的 `llm/loginOAuth` offer，并在登录页签中打开授权 URL。OpenCode Go 显示密码遮罩的密钥字段并运行 `llm/loginApiKey`；提交后的秘密会立即从浏览器状态清除，且不会进入 settings。
...
- **未声明的存活路由无处渲染**：未附带可配置提供方声明即注册的路由没有 settings 地址；它在各选择器中仍然可见，但不会出现在本页的行里。例外是已登录的托管 OAuth 路由（渲染退出登录行）及其未登录 dormant 条目（渲染“连接”卡片）。
