...
The MCP card stages the whole `servers` fleet together. Toggles, removals, and additions join one draft; saving submits `servers` wholesale in one revision-fenced mutation, so a stale editor conflicts instead of overwriting. The add form covers the common fields (name, transport, command with newline-separated args, URL); timeouts, environment, headers, and reconnect tuning live in `settings.yaml`.
