...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), the DeepSeek search provider (`web-search-deepseek`), and workspace conversation checkpoints (`workspace-checkpoint`). The checkpoint card is an explicit opt-in: its native boolean defaults to off, and enabling it turns on automatic workspace capture, recovery admission, and conversation edit/activation affordances.
