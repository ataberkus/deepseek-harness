...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、DeepSeek 搜索提供方（`web-search-deepseek`）以及工作区对话检查点（`workspace-checkpoint`）。检查点卡片是显式选择加入：原生布尔开关默认关闭，启用后才会打开自动工作区捕获、恢复准入和对话编辑/激活入口。
