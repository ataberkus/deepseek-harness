...
MCP 卡会同时暂存整个 `servers` 舰队。开关、移除与新增进入同一份草稿；保存时以一次带版本围栏的 mutation 整体提交 `servers`，因此过期编辑会冲突而不是覆盖。新增表单覆盖常用字段（名称、传输方式、命令与按行分隔的参数、URL）；超时、环境变量、请求头与重连调优仍在 `settings.yaml` 中完成。
