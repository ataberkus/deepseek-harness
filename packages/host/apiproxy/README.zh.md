...
`session.edit` 是由 Host 负责的恢复并分支事务，用于已结算的直接用户消息。它会校验前置可用检查点和源 Agent 的空闲状态，先保存紧急工作区快照，再恢复所选文件，从被编辑轮次之前的前缀创建子会话，记录源会话／紧急快照／子会话的旁车关系，并只将替换文本排入子会话；源日志保持追加写入。`session.activate` 会先保存紧急快照，再为处于空闲状态的会话恢复最新可用的非紧急检查点；没有可用文件树时返回 `{ restored: false, unavailable: true }`。当提供方存在但实时 `enabled` 设置为 `false` 时，两个 RPC 都返回 `checkpoint-disabled`。`session/checkpoints` 是可在重连时恢复的元数据和操作阶段投影，携带实时 `enabled` 标志、所选源谱系检查点、所选分支标签以及是否存在可恢复的工作区树；文件内容不会进入 mux 流。

会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 与按提供方分组的建议性模型、精确模型的推理元数据和逐提供方查询失败记录分开返回。类型化 Harness 失败会携带可选的稳定 `code`；普通提供方错误只携带诊断字段。该选择可能不在这些分组中，也绝不会作为合成行注入；客户端可以提示用户作出另一项选择，而无需把目录变成路由白名单。`session.selectModel` 校验由适配器持有的可选推理强度，并指定下次组装提示词时使用的完整选择。目录成员关系不构成校验：适配器可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。`session.models` 还会报告 `routable`，即当前是否有适配器为所选提供方提供服务。该值刻意不从分组推导，因为适配器可以服务未公布的模型。`session.prompt` 会依据同一事实，在开启轮次之前以 `model-unavailable` 拒绝；客户端禁用 composer 只是提示性设计，这个方法始终可被调用。
...
`session.create` 成功时也会回传解析后的规范 `cwd`。浏览器会立即把该值写入 Session 镜像，因此在 Host stream 追上之前，工作区相对路径的原生文件打开不会回退到进程启动目录。
