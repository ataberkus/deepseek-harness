...
`session.edit` 是由 Host 负责的恢复并分支事务，用于已结算的直接用户消息。它会校验前置可用检查点和源 Agent 的空闲状态，先保存紧急工作区快照，再恢复所选文件，从被编辑轮次之前的前缀创建子会话，记录源会话／紧急快照／子会话的旁车关系，并只将替换文本排入子会话；源日志保持追加写入。`session.activate` 会先保存紧急快照，再为处于空闲状态的会话恢复最新可用的非紧急检查点；没有可用文件树时返回 `{ restored: false, unavailable: true }`。`session/checkpoints` 是可在重连时恢复的元数据和操作阶段投影，还会携带所选分支标签以及是否存在可恢复的工作区树；文件内容不会进入 mux 流。
