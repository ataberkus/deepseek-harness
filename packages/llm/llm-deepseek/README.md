...
Each configured catalog model may include a complete `pricing` card with per-million-token `input`, `output`, `cacheRead`, and `cacheWrite` USD rates. Successful usage records retain the calculated estimate; a model without a complete rate card remains explicitly unpriced.
