...
- **Usage estimates preserve rate certainty** — pi-ai catalog pricing is retained in `TokenUsage.estimatedCostUsd`; a configured `models[].pricing` card supplies complete per-million-token rates for custom routes. Missing rates mark the session estimate unavailable rather than `$0`. Cursor uses the dated official rate snapshot in `src/cursor/pricing.ts`; `cursorTokenRate` adds the Teams/Enterprise third-party surcharge without changing first-party Cursor rates. Cursor input is approximate because AgentService reports output token deltas but not complete billed input/cache usage.

- **The Models page withholds OAuth-only catalog cards** — a key field cannot authenticate `openai-codex` or hosted `cursor`. Sign in with `/login openai-codex` (ChatGPT browser PKCE) or `/login cursor` (loginDeepControl poll); tokens persist in `$DSH_HOME/oauth-credentials.json`. Codex refresh goes through pi-ai; Cursor refresh is hosted here. The unofficial Cursor AgentService transport is not a public API. The live route appears in the model picker after login, and on the Models page as a read-only signed-in row (no key field). A settings document that already names the route keeps its directory entry so it can be edited or deleted, and `apiKeyEnv` still authenticates with that key (a token that expires unless something else refreshes it). Device-code login, a web Sign-in button, IDE tokens, and other OAuth-only catalog providers are not offered. The Web UI opens a blank tab during `/login` and navigates it on `commands/open-url`; overlapping `/login` is refused. If OpenAI's authorize page reports `missing_required_parameter`, paste the full URL written to stderr; a wrapped-link click drops the query string.
...
- **An explicit `models` list does not refresh itself** — a replacement list is whatever `settings.yaml` says, so it is only as current as its last edit. An OpenRouter catalog route that omits that list overlays a live `GET /models` listing and falls back to the installed snapshot when the endpoint is unreachable. Rows that disclose `supported_parameters` without `"tools"` are omitted. A hosted `cursor` route that omits that list overlays GetUsableModels and falls back to the bundled snapshot when Cursor is unreachable. DeepSeek and other catalog endpoints stay snapshot-only.
