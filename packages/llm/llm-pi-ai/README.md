...
- **An explicit `models` list does not refresh itself** — a replacement list is whatever `settings.yaml` says, so it is only as current as its last edit. An OpenRouter catalog route that omits that list overlays a live `GET /models` listing and falls back to the installed snapshot when the endpoint is unreachable. Rows that disclose `supported_parameters` without `"tools"` are omitted. DeepSeek and other catalog endpoints stay snapshot-only.
