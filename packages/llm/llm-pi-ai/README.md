...
- [Dev Note](#dev-note)
- [Surviving local and hosted routes](#surviving-local-and-hosted-routes)
...
The Models page offers provider-owned login without the composer. Hosted OAuth routes appear as dormant directory entries marked `auth: 'oauth'`; Connect runs the namespace `llm/loginOAuth` offer and opens the authorize URL in a prepared Web tab or the Desktop system browser. OpenCode Go appears with `auth: 'api-key'`; Connect sends its password-masked key through `llm/loginApiKey`, and pi-ai stores the resulting `api_key` credential in `$DSH_HOME/oauth-credentials.json` with owner-only permissions. Each OpenCode Go model request carries a stable opaque conversation digest as `x-opencode-session`; the raw Harness Session id stays local. Neither login creates a settings profile. Disconnecting withdraws the live route and restores its dormant entry.
...
Hosted OAuth routes remain exported for `cursor` and `google-antigravity` (shown as Cursor and Antigravity). Their login flows store credentials under `llm-pi-ai/<provider id>`, refresh them when needed, and make an OAuth-backed route available to the provider selector. The login picker also keeps provider-native key prompts separate from the settings form; signing out removes the stored route credential.
...
- **Explicit model lists remain authoritative** — catalog-backed OpenRouter routes can overlay a live listing, and hosted Cursor and Codex routes use their authenticated model listings. A configured `models` list disables those overlays; configuration-time discovery never persists candidates automatically.
