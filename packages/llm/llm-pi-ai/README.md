...
- **Explicit model lists remain authoritative** — catalog-backed OpenRouter routes can overlay a live listing, and hosted Cursor routes can use their authenticated model listing. A configured `models` list disables those overlays; configuration-time discovery never persists candidates automatically.
