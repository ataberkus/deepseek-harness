...
A profile's `models` list *replaces* the route's installed catalog rather than extending it; omitting it (or leaving it empty) serves that catalog unchanged, then overlays a live `GET /models` listing on OpenRouter catalog routes (`openrouter`, or a listable route whose listing host is `openrouter.ai` / `*.openrouter.ai`) or a GetUsableModels overlay on a hosted `cursor` route. Installed ids keep catalog metadata; tool-capable ids the snapshot does not ship are appended so the picker and request path see the endpoint's current set. A live-only OpenRouter id that discloses `reasoning` or `reasoning_effort` in `supported_parameters` is marked reasoning with the OpenRouter effort map (`low`/`medium`/`high`); other live-only ids stay non-reasoning. A live-only OpenRouter id that discloses image in `architecture.input_modalities` or `architecture.modality` is marked `[text, image]`; others stay text-only. Hosted Cursor chat families advertise `[text, image]` and send unofficial `SelectedImage` bytes; `grok-code` stays text-only. A hosted Cursor listing is live-first: GetUsableModels wins on id collision, the bundled fallback fills documented ids the reply omitted, and a documented `{id}-fast` sibling is added when Cursor ships that SKU. An explicit `models` list is not overlaid. A listing failure falls back to the installed catalog. Each entry defaults its unset fields from the installed model of the same `id`, so narrowing a catalog route to two models, correcting one capacity, or adding a model newer than the installed catalog are all one-line edits — but declaring any `models` list means every model the route should keep serving must appear in it, an entry of nothing but `id` being enough. The configurable entry fields are `id`, `name`, `contextWindow`, `maxTokens`, `reasoningEfforts`, and `compat`. Pricing and input modalities have no harness consumer and ride the installed entry or are absent.
...
- **An explicit `models` list does not refresh itself** — a replacement list is whatever `settings.yaml` says, so it is only as current as its last edit. An OpenRouter catalog route that omits that list overlays a live `GET /models` listing and falls back to the installed snapshot when the endpoint is unreachable. Rows that disclose `supported_parameters` without `"tools"` are omitted. Live-only rows that name `reasoning` or `reasoning_effort` get the OpenRouter effort map; others stay non-reasoning. Live-only rows that disclose image in `architecture` get `[text, image]`; others stay text-only. A hosted `cursor` route that omits that list overlays GetUsableModels live-first, fills documented ids (including Fast SKUs) the reply omitted, and falls back to the bundled snapshot when Cursor is unreachable. Cursor chat families advertise image input; `grok-code` does not. DeepSeek and other catalog endpoints stay snapshot-only.
