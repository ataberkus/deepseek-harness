...
- **The Models page withholds OAuth-only catalog cards** — a key field cannot authenticate `openai-codex` or hosted `cursor`. Sign in with `/login openai-codex` (ChatGPT browser PKCE) or `/login cursor` (loginDeepControl poll); tokens persist in `$DSH_HOME/oauth-credentials.json`. Codex refresh goes through pi-ai; Cursor refresh is hosted here. The unofficial Cursor AgentService transport is not a public API. The live route appears in the model picker after login, and on the Models page as a signed-in row with Sign out (`llm.logout`, same store delete as `/logout openai-codex` / `/logout cursor`). A settings document that already names the route keeps its directory entry so it can be edited or deleted, and `apiKeyEnv` still authenticates with that key (a token that expires unless something else refreshes it). Device-code login, a web Sign-in button, IDE tokens, and other OAuth-only catalog providers are not offered. The Web UI opens a blank tab during `/login` and navigates it on `commands/open-url`; overlapping `/login` is refused. If OpenAI's authorize page reports `missing_required_parameter`, paste the full URL written to stderr; a wrapped-link click drops the query string.
