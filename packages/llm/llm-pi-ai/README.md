...
Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. Each profile may set a `retryPolicy`; omission uses normal mode with five retries. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. Omitting it leaves the route unauthenticated, which for an installed catalog route means pi-ai's provider-native ambient discovery; the first-class `lmstudio` route instead supplies an internal non-secret placeholder key because pi-ai's OpenAI client requires a key-shaped input even when the local server does not authenticate. A configured reference that resolves to nothing fails the request with `MISSING_CREDENTIAL`, because falling through would authenticate with whatever unrelated key the environment happens to hold. One credential serves every model on its route.
...
LM Studio uses `openai-completions` at `http://127.0.0.1:1234/v1` by default. The Models page can ask `GET /v1/models`, presents the returned ids without rewriting them, and saves the adopted list in the profile; it does not poll or replace the persisted list on every request. If LM Studio authentication is enabled, set `apiKeyEnv` to a real credential reference and that value replaces the placeholder. The dict shape makes duplicate routes unrepresentable, and the pre-release array shape (with per-profile `provider` fields) fails load with migration directions. `providers` may also be empty or omitted entirely: the adapter then mounts **dormant** — zero routes, no extra catalog entries — and registers routes the moment the `llm-pi-ai:` settings section supplies profiles, dropping them again when it empties. Dormant or not, the plugin declares every installed catalog provider in the configurable-provider directory (`ctx.llm.listConfigurableProviders()`, settings path `providers.<provider>`), joined with every route the current profiles declare, so configuration surfaces can offer the full catalog before any route exists and can still address a hand-declared one. Each entry carries `declared`: whether pi-ai ships nothing under that key. It follows the installed catalog, never the settings document, because narrowing a shipped provider's models stores a profile too and that route is still one pi-ai knows — only the adapter can tell the two apart, which is why the directory answers rather than leaving a surface to infer it. Which adapters exist is composition; which providers run can be entirely the user's settings document. Registration with `ctx.llm` is atomic: a collision with any provider route already owned by another adapter fails plugin loading without registering the remaining routes. Model ids are not lifecycle config; a model the route does not configure fails before any provider request with `LlmError('UNKNOWN_MODEL')`.

## Catalog resolution

Hosted Cursor model discovery sends its unary `GetUsableModels` request as raw protobuf with `application/proto`; `AgentService/Run` remains Connect-framed. Non-2xx HTTP responses are treated as transport failures, so the bundled fallback remains available instead of being reported as a successful empty catalog.

A profile's `models` list *replaces* the route's installed catalog rather than extending it; omitting it (or leaving it empty) serves that catalog unchanged, then overlays a live `GET /models` listing on OpenRouter catalog routes (`openrouter`, or a listable route whose listing host is `openrouter.ai` / `*.openrouter.ai`) or a GetUsableModels overlay on a hosted `cursor` route. Installed ids keep catalog name and capacities; a live OpenRouter row's `reasoning.supported_efforts` and `default_effort` replace that id's snapshot effort map. Tool-capable ids the snapshot does not ship are appended so the picker and request path see the endpoint's current set. A live-only OpenRouter id that discloses a `reasoning` object or a reasoning parameter is marked reasoning with those efforts (a parameter without a usable object uses `low`/`medium`/`high`); other live-only ids stay non-reasoning. A live-only OpenRouter id that discloses image in `architecture.input_modalities` or `architecture.modality` is marked `[text, image]`; others stay text-only. A hosted Cursor listing is live-first: GetUsableModels wins on id collision, the bundled fallback fills documented ids the reply omitted, and a documented `{id}-fast` sibling is added when Cursor ships that SKU. Reasoning Cursor models attach a family effort map (Fast suffix stripped) because GetUsableModels `ThinkingDetails` is a presence flag and does not name efforts. Hosted Cursor chat families advertise `[text, image]` and send unofficial `SelectedImage` bytes; `grok-code` stays text-only. An explicit `models` list is not overlaid. A transport failure falls back to the installed catalog; a successful empty GetUsableModels response raises `CURSOR_NO_USABLE_MODELS` and leaves the provider in the catalog failures list. Each entry defaults its unset fields from the installed model of the same `id`, so narrowing a catalog route to two models, correcting one capacity, or adding a model newer than the installed catalog are all one-line edits — but declaring any `models` list means every model the route should keep serving must appear in it, an entry of nothing but `id` being enough. The configurable entry fields are `id`, `name`, `contextWindow`, `maxTokens`, `reasoningEfforts`, and `compat`. Pricing and input modalities have no harness consumer and ride the installed entry or are absent.
...
The plugin offers `ctx.llm.registerModelDiscovery('llm-pi-ai', …)`, which answers "which models can this provider serve?" for a route a configuration surface is editing or drafting. It is deliberately *not* a stored catalog refresh: nothing is written, and the reply is candidates the surface offers for adoption. `settings.yaml` remains the only thing that decides what a route's explicit `models` list serves. A route that omits that list still overlays the same live listing the picker uses.

A request naming a route the **installed catalog ships is answered from that catalog**. OpenRouter catalog routes overlay the endpoint listing (the `openrouter` catalog id, or a listable route whose listing host is `openrouter.ai` / `*.openrouter.ai`): catalog ids keep catalog capacities, and tool-capable ids the snapshot does not ship are appended. Other catalog endpoints are not listed — they share the inference URL, so a `GET /models` there is not a catalog refresh. A listing failure falls back to the catalog, so Fetch still returns the installed set. Such a route needs no `baseURL` at all — the catalog provider's endpoint is used. Only a route the catalog does not describe — a gateway, a self-hosted server — is interrogated with no catalog floor, and one that names no endpoint is told to set one or enter its models by hand.
...
API-key credentials never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override, so a named `apiKeyEnv` keeps fail-loud reference semantics. OAuth credentials use the collection's `CredentialStore`: the plugin constructs `createModels({ credentials })` with `$DSH_HOME/oauth-credentials.json`, and `/login openai-codex` / `/login cursor` / `/login google-gemini-cli` are the host logins that fill it. A route naming no credential resolves as configured-but-keyless and leaves the requirement to the protocol, which is where it actually lives — except `lmstudio`, whose resolver supplies the non-secret placeholder, and `openai-codex`, hosted `cursor`, and hosted `google-gemini-cli`, which authenticate from that store after login.
...
- pi-ai reports failures as in-stream error events; these map to `finish {kind:'error'|'aborted', failure}` chunks. Provider-specific error text distinguishes terminal `QUOTA` from transient `RATE_LIMIT`, while text and usage signals evaluated against the resolved model's context window normalize overflow to `CONTEXT_WINDOW_EXCEEDED`. A terminal `stop` whose message carries no content blocks maps to a `finish {kind:'error'}` with code `CURSOR_EMPTY_STREAM` for Cursor (outside the default retry policy) and `EMPTY_RESPONSE` for other providers (retried by default) instead of a successful empty message.
- pi-ai folds reasoning tokens into output usage; there is no separate reasoning count to map.
- A positive pi-ai catalog total is carried as `TokenUsage.costUsd` for the live call; zero-rate `NO_COST` models omit spend, and replay reconstruction keeps its zero-cost usage.
...
- **The Models page withholds OAuth-only catalog cards** — a key field cannot authenticate `openai-codex`, hosted `cursor`, or hosted `google-gemini-cli`. Sign in with `/login openai-codex` (ChatGPT browser PKCE), `/login cursor` (loginDeepControl poll), or `/login google-gemini-cli` (Google auth-code on `127.0.0.1:8085`); tokens persist in `$DSH_HOME/oauth-credentials.json`. Codex refresh goes through pi-ai; Cursor and Gemini CLI refresh are hosted here. The unofficial Cursor AgentService and Cloud Code Assist transports are not public APIs. Workspace Gemini accounts may need `GOOGLE_CLOUD_PROJECT`. The live route appears in the model picker after login, and on the Models page as a signed-in row with Sign out (`llm.logout`, same store delete as `/logout openai-codex` / `/logout cursor` / `/logout google-gemini-cli`). A settings document that already names the route keeps its directory entry so it can be edited or deleted, and `apiKeyEnv` still authenticates with that key (a token that expires unless something else refreshes it). Device-code login, a web Sign-in button, IDE or Gemini CLI on-disk tokens, `google-antigravity`, and other OAuth-only catalog providers are not offered. The Web UI opens a blank tab during `/login` and navigates it on `commands/open-url`; the host uses the OS browser only when no `commands/open-url` subscriber exists, and overlapping `/login` is refused. If OpenAI's authorize page reports `missing_required_parameter`, paste the full URL written to stderr; a wrapped-link click drops the query string.
...
- **An explicit `models` list does not refresh itself** — a replacement list is whatever `settings.yaml` says, so it is only as current as its last edit. An OpenRouter catalog route that omits that list overlays a live `GET /models` listing and falls back to the installed snapshot when the endpoint is unreachable. Rows that disclose `supported_parameters` without `"tools"` are omitted. Live rows take effort names from the listing `reasoning` object when present; a reasoning parameter without that object uses `low`/`medium`/`high`; matching installed ids take the live map; others stay non-reasoning. Live-only rows that disclose image in `architecture` get `[text, image]`; others stay text-only. A hosted `cursor` route that omits that list overlays GetUsableModels live-first, fills documented ids (including Fast SKUs) the reply omitted, attaches family effort maps on reasoning ids, network failures keep the bundled snapshot, while a successful empty reply produces `CURSOR_NO_USABLE_MODELS` in the provider catalog failures. Cursor chat families advertise image input; `grok-code` does not. A heartbeat-only Cursor completion produces `CURSOR_EMPTY_STREAM` and is outside the default retry policy. Direct DeepSeek and other non-OpenRouter catalog endpoints stay snapshot-only.
...
- **Generic unauthenticated OpenAI-compatible routes need an auth input** — pi-ai's OpenAI-compatible implementation requires an API key or an `Authorization` header. The named `lmstudio` route is the exception: with no `apiKeyEnv`, the adapter supplies a non-secret placeholder automatically; explicit credentials still take precedence.
