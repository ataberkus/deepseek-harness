...
托管 Cursor 的模型发现会以原始 protobuf 和 `application/proto` 发送一元 `GetUsableModels` 请求；`AgentService/Run` 仍使用 Connect 分帧。非 2xx HTTP 响应会被视为传输失败，因此会保留捆绑 fallback，而不会被误报为成功但为空的 catalog。

profile 的 `models` 列表是*替换*该路由已安装 catalog，而不是扩充它；省略它（或留空）则原样服务该 catalog，并在 OpenRouter catalog 路由（`openrouter`，或列举主机名为 `openrouter.ai`／`*.openrouter.ai` 的可列举路由）上叠加上一份实时 `GET /models` 列表，或在托管的 `cursor` 路由上叠加 GetUsableModels。已安装 id 保留 catalog 名称与容量；实时 OpenRouter 行的 `reasoning.supported_efforts` 与 `default_effort` 会替换该 id 的快照档位映射。快照未收录、且具备工具能力的 id 会追加，因此选择器与请求路径看到的是端点当前集合。仅出现在实时列表中的 OpenRouter id 若公布了 `reasoning` 对象或推理参数，会被标为具备推理能力并带上这些档位（有参数却没有可用对象时使用 `low`／`medium`／`high`）；其余仅实时 id 仍不具备推理能力。仅实时 OpenRouter id 若在 `architecture.input_modalities` 或 `architecture.modality` 中公布了图片输入，会被标为 `[text, image]`；其余仍为纯文本。托管 Cursor 列表以实时为准：GetUsableModels 在 id 冲突时胜出，捆绑 fallback 补上回复遗漏的已记录 id，并且当 Cursor 提供对应 SKU 时补上已记录的 `{id}-fast` 兄弟项。具备推理的 Cursor 模型会挂上按家族划分的档位映射（去掉 Fast 后缀），因为 GetUsableModels 的 `ThinkingDetails` 只是存在标志、并不点名档位。托管 Cursor 的聊天家族宣称 `[text, image]` 并发送非官方 `SelectedImage` 字节；`grok-code` 仍为纯文本。只有 heartbeat 的 Cursor 完成会产生 `CURSOR_EMPTY_STREAM`，且不在默认重试策略内。显式 `models` 列表不会被叠加。网络列举失败则回退到已安装 catalog；成功但为空的 GetUsableModels 响应会以 `CURSOR_NO_USABLE_MODELS` 出现在提供方目录失败记录中。每个条目都会从同 `id` 的已安装模型继承自身未设置的字段，因此把 catalog 路由收窄到两个模型、更正某个容量，或加入一个比已安装 catalog 更新的模型，都是一行编辑——但一旦声明了 `models` 列表，该路由要继续服务的每个模型就都必须出现在其中，条目哪怕只写一个 `id` 也足够。可配置的条目字段是 `id`、`name`、`contextWindow`、`maxTokens`、`reasoningEfforts` 与 `compat`。定价与输入模态没有 harness 消费方，因此沿用已安装条目或直接缺席。
...
插件提供 `ctx.llm.registerModelDiscovery('llm-pi-ai', …)`，用来回答「这个提供方能服务哪些模型？」——针对配置界面正在编辑或起草的路由。它刻意**不是**一次写入配置的 catalog 刷新：什么都不写入，回复是界面供用户采纳的候选。`settings.yaml` 仍然是唯一决定路由显式 `models` 列表服务什么的东西。省略该列表的路由仍会叠加上选择器已经使用的那份实时列表。

点名了**已安装 catalog 所提供路由**的请求，由该 catalog 作答。OpenRouter catalog 路由会叠加上端点列表（`openrouter` catalog id，或列举主机名为 `openrouter.ai`／`*.openrouter.ai` 的可列举路由）：catalog id 保留 catalog 容量；快照未收录、且具备工具能力的 id 会追加。其他 catalog 端点不会被列举——它们与推理共用同一 URL，因此那里的 `GET /models` 并不是一次 catalog 刷新。列举失败则回退到 catalog，因此 Fetch 仍会返回已安装集合。这类路由根本不需要 `baseURL`——使用 catalog 提供方的端点。只有 catalog 未描述的路由——网关、自建服务——才会在没有 catalog 底的情况下询问；若它也没给端点，则会被告知去设置一个或手工填写模型。
...
路由的 `apiKeyEnv` 密钥仍在请求抵达 pi-ai 之前经 harness 自身 seam 解析，并作为请求的 `apiKey` 选项传入——pi-ai 将其视为优先级最高的 auth 覆盖，harness 因此保住自己明确失败的引用语义；命名的 `lmstudio` 路由由解析器在未配置显式引用时自动提供非机密占位值。在该覆盖之下，集合携带本插件的凭据存储与 ambient auth context：已存储的登录（OAuth grant，或在 pi-ai 自己的登录提示里键入的密钥）经由它们为路由完成认证，并在存储的跨进程锁下自行刷新。存储以 `llm-pi-ai/<provider id>` 为记录地址；手写路由键若落在记录文法之外（大写、点、下划线），读取时视为「没有存储任何东西」而不是寻址错误——这样的路由无法登录（对它的记录写入会以 `LlmError('UNSTORABLE_PROVIDER_ID')` 拒绝），只能经 `apiKeyEnv` 或提供方 ambient 设置认证。托管的 `cursor` 与托管的 `google-gemini-cli` 在登录后从该 store 认证。完全没有点名任何凭据的路由会解析为「已配置但无密钥」，把该要求留给协议——那才是它真正所在的位置。
...
- pi-ai 将失败报告为流内错误事件；它们会映射到 `finish {kind:'error'|'aborted', failure}` 分片。提供方特定错误文本会区分终止型 `QUOTA` 与暂时型 `RATE_LIMIT`，针对已解析模型上下文窗口评估的文本与 usage 信号则将溢出规范化为 `CONTEXT_WINDOW_EXCEEDED`。终止时的 `stop` 若消息不含内容块，Cursor 会映射为 `finish {kind:'error'}`，code 为 `CURSOR_EMPTY_STREAM`（不在默认重试策略内）；其他提供方则使用 `EMPTY_RESPONSE`（默认策略会重试），而非成功空消息。
- pi-ai 将推理 token 折叠到输出 usage 中；没有可映射的独立推理计数。
- 正的 pi-ai 目录总价会作为实时调用的 `TokenUsage.costUsd` 传递；零价格的 `NO_COST` 模型省略花费，回放重建仍保留零花费 usage。
...
- **模型页不提供仅 OAuth 的 catalog 卡片**：密钥字段无法认证 `openai-codex`、托管的 `cursor` 或托管的 `google-gemini-cli`。使用 `/login openai-codex`（ChatGPT 浏览器 PKCE）、`/login cursor`（loginDeepControl 轮询）或 `/login google-gemini-cli`（`127.0.0.1:8085` 上的 Google 授权码）登录；token 持久化在 `$DSH_HOME/oauth-credentials.json`。Codex 刷新走 pi-ai；Cursor 与 Gemini CLI 刷新由本宿主负责。非官方 Cursor AgentService 与 Cloud Code Assist 传输不是公开 API。Workspace Gemini 账号可能需要 `GOOGLE_CLOUD_PROJECT`。登录后 live 路由出现在模型选择器中，并在模型页显示为已登录行，带退出登录（`llm.logout`，与 `/logout openai-codex`／`/logout cursor`／`/logout google-gemini-cli` 删除同一份存储）。settings 文档已经点名的路由仍保留目录条目，以便编辑或删除；`apiKeyEnv` 也仍能用该密钥认证（若没有其他环节刷新，该 token 会过期）。不提供 device-code 登录、Web「登录」按钮、IDE 或 Gemini CLI 磁盘 token、`google-antigravity`，以及其他仅 OAuth 的 catalog 提供方。Web UI 在 `/login` 时打开空白标签，并在 `commands/open-url` 上导航；只有不存在 `commands/open-url` 订阅者时宿主才使用操作系统浏览器，重叠的 `/login` 会被拒绝。若 OpenAI 授权页报告 `missing_required_parameter`，把 stderr 写出的完整 URL 粘贴到地址栏；点击被换行截断的链接会丢掉查询串。
