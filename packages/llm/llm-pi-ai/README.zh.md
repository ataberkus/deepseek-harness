...
- [开发备注](#dev-note)
- [保留的本地与托管路由](#surviving-local-and-hosted-routes)
...
Models 页面无需经过对话输入框即可完成提供方自有登录。托管 OAuth 路由以带 `auth: 'oauth'` 标记的 dormant 目录条目出现；“连接”会运行命名空间的 `llm/loginOAuth` offer，并在已准备的 Web 页签或 Desktop 系统浏览器中打开授权 URL。OpenCode Go 以 `auth: 'api-key'` 出现；“连接”通过 `llm/loginApiKey` 发送密码遮罩的密钥，pi-ai 将得到的 `api_key` 凭据以仅属主可访问的权限存入 `$DSH_HOME/oauth-credentials.json`。每个 OpenCode Go 模型请求都会将稳定的不透明会话摘要作为 `x-opencode-session` 发送；原始 Harness Session ID 保留在本地。两种登录都不会创建 settings profile。断开连接会收回 live 路由并恢复其 dormant 条目。

<a id="surviving-local-and-hosted-routes"></a>
### 保留的本地与托管路由

本包仍提供一等的 `lmstudio` 路由，默认使用 `openai-completions` 与 `http://127.0.0.1:1234/v1`。未设置 `apiKeyEnv` 时，适配器会为 pi-ai 的 OpenAI 客户端提供非秘密占位符；如果 LM Studio 启用了认证，则应改为配置凭据引用。Models 界面可以通过本地 `/v1/models` 发现已加载的模型 id，并把采用的 id 保存到 profile。

托管 OAuth 路由仍导出 `cursor` 与 `google-antigravity`（界面名称分别为 Cursor 与 Antigravity）。登录流程把凭据存储在 `llm-pi-ai/<provider id>` 下，并在需要时刷新，使 OAuth 路由可供提供方选择器使用。登录选择器也会把提供方原生密钥提示与设置表单分开；退出登录会删除已存储的路由凭据。
...
- **显式模型列表具有最终决定权**——基于目录的 OpenRouter 路由可叠加实时列表，托管 Cursor 与 Codex 路由可使用各自的认证模型列表。配置 `models` 列表会禁用这些叠加；配置时的模型发现不会自动持久化候选。
