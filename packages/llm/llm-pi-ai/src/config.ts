 ...
 * not, the profile is the whole provider declaration. The named `lmstudio`
 * route supplies local endpoint and protocol defaults with explicit models. Stored reads retain
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    const { api: _api, baseURL: _baseURL, apiKeyEnv, retryPolicy, models: _models, displayName: _displayName, ...rest } = source
