...
- `ctx.llm.logout(provider: string): Promise<void>` Sign out of a hosted OAuth route (`openai-codex`, `cursor`, or `google-antigravity`). Adapters that do not persist OAuth credentials reject with `UNSUPPORTED_OPTION`.
