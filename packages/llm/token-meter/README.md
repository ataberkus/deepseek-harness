...
`tokenUsage` also carries `estimatedCostUsd`, `unpricedSteps`, and `approximateSteps`. A priced usage sample replaces an earlier sample for the same step together with its cost status; a usage-bearing sample without a complete rate card increments `unpricedSteps`. These fields preserve the estimate recorded with the historical usage instead of repricing old sessions.
