...
describe('tokenUsage session projection', () => {
  ...
  it('sums cost across steps and replaces a same-step sample', async () => {
    const { ctx, session } = await harness()
    startStep(session, 1, 1)
    usageChunk(session, { inputTokens: 10, outputTokens: 2, costUsd: 0.10 }, 1, 1)
    finalUsage(session, { inputTokens: 12, outputTokens: 3, costUsd: 0.12 }, 1, 1)
    startStep(session, 1, 2)
    usageChunk(session, { inputTokens: 20, outputTokens: 4, costUsd: 0.05 }, 1, 2)
    finalUsage(session, { inputTokens: 20, outputTokens: 4, costUsd: 0.05 }, 1, 2)
