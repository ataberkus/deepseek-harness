...
Checkpoint metadata is not a `SessionEvent` and never enters the system prompt or derived model history. `Checkpoint 0` uses `boundarySeq: -1` for the workspace before the first turn.

## Service API (`ctx.workspaceCheckpoint`)

| Member | Semantics |
|---|---|
| `capture(request)` | Snapshot the session cwd. Capture is fail-soft: an unavailable record does not erase a completed turn. |
