...
检查点元数据不是 `SessionEvent`，不会进入 system prompt 或派生的模型历史。`Checkpoint 0` 使用 `boundarySeq: -1` 表示第一回合之前的工作区。

## 服务 API（`ctx.workspaceCheckpoint`）

| 成员 | 语义 |
|---|---|
| `capture(request)` | 快照会话 cwd。捕获是 fail-soft：unavailable 记录不会抹掉已完成的回合。 |
