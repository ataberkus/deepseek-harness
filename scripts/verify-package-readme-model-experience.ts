...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/session/workspace-checkpoint': { kind: 'none', reason: 'The service definition owns checkpoint records and lifecycle methods; it registers nothing model-facing.' },
  'packages/session/workspace-checkpoint-capture': { kind: 'none', reason: 'The consumer guards model and tool dispatch but registers no prompt, schema, or tool of its own.' },
  'packages/session/workspace-checkpoint-local': { kind: 'none', reason: 'The local checkpoint provider persists workspace files and registers nothing model-facing.' },
